...
import { ThemeToken, ThemeComponents, ThemeComponent } from './types/ThemeSource';

const convertTokensToStyles = (tokens: ThemeToken[], variables: Record<string, unknown>): StyleProperties => {
...
export const getThemeComponents = <C extends string>(
    ...
    const themeComponents: Record<C, (props: object) => StyleProperties> = {} as Record<
        C,
    ...
    for (const [name, component] of Object.entries(components)) {
        themeComponents[name as C] = (props: object) => {
            const tokens = (component as (props: object) => ThemeComponent)(props).tokens;
            const variables = getVariables(tokens);
            return convertTokensToStyles((component as (props: object) => ThemeComponent)(props).tokens, variables);
        };
    }
